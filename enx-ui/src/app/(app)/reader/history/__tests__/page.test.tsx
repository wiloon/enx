...
it('lists saved documents newest first, showing each one\'s content preview', async () => {
      ...
      documents: [
        {
          ...
          preview: 'Second article preview',
        },
        {
          id: 'doc-1',
          createdAt: '2026-09-14T00:00:00Z',
          updatedAt: '2026-09-14T00:00:00Z',
          preview: 'First article preview',
        },
      ],
    },
  })
  ...
  const rows = await screen.findAllByRole('button', { name: /article preview/ })
  expect(rows).toHaveLength(2)
  expect(rows[0]).toHaveTextContent('Second article preview')
  expect(rows[1]).toHaveTextContent('First article preview')
})

it('falls back to "Untitled" when a document has no preview text', async () => {
  mockList.mockResolvedValue({
    success: true,
    data: {
      documents: [
...
it('deletes a document and removes it from the list', async () => {
  ...
  renderPage()
  await screen.findByRole('button', { name: /doomed document/ })
