...
func parseResponse(jsonBody string) *Response {
	...
	// Fall back to translation array when basic.explains is absent (isWord:false case)
	if youdaoResponse.BasicExplains == "" {
		translations := gjson.Get(jsonBody, "translation").Array()
		...
		if len(parts) > 0 {
			youdaoResponse.BasicExplains = strings.Join(parts, "; ")
		}
	}
