...
describe('Word Highlighting Integration Test', () => {
  it('should process a complete English sentence and generate highlight markup', () => {
    ...
    Object.keys(mockWordData).forEach(word => {
      ...
      console.log(
        `🎨 Word "${word}": color=${color}, highlight=${shouldHighlight}`
      )
    })
    ...
    Object.keys(mockWordData).forEach(word => {
      ...
      if (color !== '#FFFFFF') {
        ...
        processedSentence = processedSentence.replace(regex, match => {
          return `<u class="enx-word enx-${word}" data-word="${match}" style="text-decoration: ${color} underline; text-decoration-thickness: 1px;">${match}</u>`
        })
      }
    })
    ...
    expect(processedSentence).toContain('text-decoration-thickness: 1px')

    console.log('✅ Integration test completed successfully!')
    console.log(
      `📈 Results: ${highlightCount} words highlighted out of ${Object.keys(mockWordData).length} processed`
    )
