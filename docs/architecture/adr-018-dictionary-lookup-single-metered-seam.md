# ADR-018：查词收敛成一个计量 seam——`words`/`user_dicts` + ECDICT 封在 `dictionary.Lookup` 之下，每次调用计一次每日配额（不去重、缓存命中也算），SQLite 单条 upsert 计数，配额存储出错时 fail-open

| 字段 | 值 |
| --- | --- |
| **状态** | Proposed — 2026-09-06 |
| **日期** | 2026-09-06 |
| **关联 Spec** | [`TASK-SPEC-enx-billing-stripe-subscription.md`](../tasks/TASK-SPEC-enx-billing-stripe-subscription.md) §4.2 已把 `dictionary.Lookup` 定位为「统一查词入口，在返回结果前插入配额检查」——本 ADR 是**把这个意图补齐**（实现时 `fillFromEcdict` 把「先查本地」的分支留在了 seam 外，`DoSearchEcdict` 又各写一套，见 Context）；配套 TASK-SPEC 增补留到编码阶段（同 ADR-008 / ADR-011 / ADR-017 的做法） |
...
| **关联 Issue** | 解决 [#16](https://github.com/wiloon/enx/issues/16)（配额先扣后查——本 ADR 确认这是有意的）、[#17](https://github.com/wiloon/enx/issues/17)（非 sentinel error 被当「查无此词」）、[#18](https://github.com/wiloon/enx/issues/18)（`isActiveSubscriber` 吞错误、订阅者被降级 429）、[#19](https://github.com/wiloon/enx/issues/19)（`/ecdict` 通用错误返回 200 + null）、[#20](https://github.com/wiloon/enx/issues/20)（`/ecdict` 对已存词计量、与翻译路径不一致） |
...
**翻译路径**（`GET /translate`、`GET /word/:word` → `translate/service.go` 的 `translateWord`）：
...
**搜索框路径**（`GET /ecdict` → `enx-api.go:305` 的 `DoSearchEcdict`）：直接 `dictionary.Lookup(ctx, key, userID)`，没有「先查本地」这一步，**每次都走配额**。

`dictionary.Lookup`（`dictionary/lookup.go`）对非订阅用户无条件 `quota.CheckAndIncrementLookup`。结果：同一个用户、同一个词，查过一次之后再查，**费用取决于他从哪个入口点进去**，且搜索框那边偏贵。

### seam 在错的位置

TASK-SPEC-billing §4.2 写的是「`dictionary.Lookup` = 统一查词入口」。实现时没做到：`dictionary.Lookup` 名字像「查一个词」，实际只是「查 ECDICT + 配额」的薄包装——「先查本地库、本地没有再查 ECDICT」这个决策留在每个 caller 里（`fillFromEcdict` 自己判 `word.Id != ""` 提前返回，`DoSearchEcdict` 干脆不判）。应用层本该只说「查这个词」、不关心命中的是本地缓存还是 ECDICT，现在做不到，配额也就跟着漏在了「本地命中」这条路径上。

### 这条路径的错误处理很脆（#17 / #18 / #19）
...
- `DoSearchEcdict` 对通用错误不处理，`dict` 为 nil 时仍 `c.JSON(200, {Dict: null})`——客户端分不清「服务器错了」和「真没这个词」。
...
| 方案 | 做法 | 结论 |
| --- | --- | --- |
| A1. 现状：`dictionary.Lookup` = ECDICT-only 包装，「先查本地」留在每个 caller | `fillFromEcdict` 判 `word.Id`，`DoSearchEcdict` 不判 | 就是 #20 的来源；seam 太浅，每个 caller 都要重复（或忘记）同一段编排 |
| **A2.（采用）`dictionary.Lookup(ctx, word, userID)` = 完整的「查一个词」** | 内部：本地 `words`/`user_dicts` → 命中即返回；未命中再查 ECDICT。返回统一结果（释义 + 一个 `source` 标记，caller 一般不看）。**两个 caller 都只调它，不再各自编排** | 应用层只说「查这个词」；计量点只有一个（见 B）；`fillFromEcdict` 那段「判 `word.Id` / 分两支」的逻辑收进 seam 内 |
...
### D. 并发下的 check + increment（修 #15/#17/#18 的根）
...
| #19 | `DoSearchEcdict` 加 `else if err != nil { 5xx; return }` 分支，放在 `c.JSON(200, result)` 之前 |
...
1. **`dictionary.Lookup(ctx, word, userID)` 变成完整的查词 seam**（采用 A2）：
   ...
   - `translate/service.go` 的 `translateWord` 和 `enx-api.go` 的 `DoSearchEcdict` 都只调这一个函数。`fillFromEcdict` 里「判 `word.Id` / 分两支 / 写 `words` + `user_dicts`」的编排收进 seam（`user_dicts.QueryCount` 那套复习计数逻辑保持不变，只是搬位置）。
   - 具体返回结构（是否复用 `enx.Word` / `enx.Dictionary`、`source` 字段形态）留给 TASK-SPEC。

2. **计量：每次调用 `+1`，不去重，缓存命中也算**（采用 B2）。非订阅用户每进入一次 `dictionary.Lookup` 且拿到结果（`local` 或 `ecdict`），配额 `+1`。订阅用户跳过。**先扣后查**（#16 确认为有意）。澄清 ADR-009 Decision 6：配额是「释义查询」的每日封顶，不区分数据来源。
...
- **A2（深 seam）而不是 A1**：`dictionary.Lookup` 只有两个 caller，把「先本地后 ECDICT + 计量」这段编排放进去，两个 caller 从「各自拼装、其中一个还拼错了」变成「都只说查词」。#20 不是加个判断能根治的，是 seam 位置的问题——计量逻辑跟着数据源走，就必然有人漏掉某条路径。
...
- 一个 seam、一个计量点。#16 / #20 从设计上消失，不靠「记得在每条路径都加判断」。
- #17 / #18 / #19 随重构一并修：基建抖动不再表现为「查无此词」，付费订阅者不再被 DB 抖动降级 429，`/ecdict` 错误有正确的 5xx。
...
- **`dictionary.Lookup` 重构要动 `fillFromEcdict` / `translateWord` 的结构**，以及它们和 `dictionary` / `translate` 包的测试；`user_dicts.QueryCount` 复习计数逻辑搬位置时要保证行为逐字节不变（有测试覆盖）。
...
- 实施顺序建议：
  ...
  2. `isActiveSubscriber` 返 `(bool, error)` + `dictionary.Lookup` 错误路径修（#17/#18），`DoSearchEcdict` 加 5xx 分支（#19）。此步不改 seam 形状，可独立上线。
  3. `dictionary.Lookup` 收敛成深 seam（A2）：内部并本地 + ECDICT，两个 caller 改为只调它，计量口径统一（B2）。`fillFromEcdict` 删除或退化成 `dictionary.Lookup` 的薄 adapter。
  4. 配额行清理 job（G）。
  5. 每步独立可验证、可回滚。
...
- **`user_dicts.QueryCount` 复习计数机制**本身不变，只随 seam 重构搬位置。
