...
| **关联代码** | **待实现。** 预期落点——阶段 1（enx-ui 前端）：`src/app/(app)/app/page.tsx`（重写）、`src/components/app/{app-nav.ts,AppSidebar.tsx,AppTopbar.tsx}`、`src/components/ui/button.tsx`（加 `brand` variant）、新增 `src/components/app/home/*`。阶段 2（enx-api）：新薄包或 `word/` 下新增 `stats_overview.go` + `enx-api.go` 注册 `GET /api/stats/overview`；enx-ui `src/services/api.ts` + `src/types/index.ts`。 |
