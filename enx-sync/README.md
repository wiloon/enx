...
> ## ⚠️ 暂停维护（2026-09-17）
>
> **enx-sync（P2P 同步）暂时废弃，未来是否恢复不确定。**
>
> 在此期间，**所有新需求、设计和 ADR 一律不考虑 P2P 同步**——不要再把
> 「这张表要经 P2P 同步出去」当成设计约束。`words` 表的 UUID 主键、
> Unix 毫秒时间戳、软删除等 P2P 兼容特性保持原样（不回滚），但它们
> 现在只是历史形态，不构成新设计的限制。
>
> 代码保留在仓库里，不删除。恢复维护前请先更新本说明。
